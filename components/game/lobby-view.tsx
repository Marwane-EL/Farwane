  ...
  return (
    <div className="h-full w-full overflow-y-auto">
      <div className="flex flex-col items-center px-4 py-6 sm:py-10 min-h-full">
        ...
        {/* Room code */}
        <div className="text-center mb-6 sm:mb-8 animate-in fade-in slide-in-from-top-4 duration-500 shrink-0">
          <p className="text-muted-foreground text-sm mb-2">Code de la partie</p>
          <div className="flex items-center justify-center gap-2 sm:gap-3">
            <h1 className="text-4xl sm:text-6xl md:text-7xl font-black tracking-[0.3em] bg-gradient-to-r from-secondary via-primary to-accent bg-clip-text text-transparent">
              {roomCode}
            </h1>
            <Button variant="ghost" size="icon" onClick={copyCode} className="h-10 w-10 sm:h-12 sm:w-12 rounded-full hover:bg-muted transition-colors shrink-0">
            ...
            <Card className="border-2 border-primary/50 bg-gradient-to-r from-primary/10 to-accent/10">
              <CardContent className="p-3 sm:p-4">
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <span className="font-semibold flex-1 min-w-[80px]">Pack de Memes :</span>
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button variant="outline" className="min-w-[160px] sm:min-w-[180px] justify-between border-2 border-border hover:border-primary h-9 sm:h-10 text-sm">
                  ...
                  <div className="flex gap-2">
                    {timerOptions.map((t) => (
                      <Button
                        ...
                        className={`flex-1 h-8 sm:h-9 font-bold transition-all text-xs sm:text-sm ${settings.timerDuration === t
                  ...
                  <div className="flex gap-2">
                    {roundOptions.map((r) => (
                      <Button
                        ...
                        className={`flex-1 h-8 sm:h-9 font-bold transition-all text-xs sm:text-sm ${settings.totalRounds === r
                            ? "bg-secondary text-secondary-foreground shadow-md"
                            : "hover:border-secondary"
                          }`}
                      >
        ...
        {!isHost && (
          <Card className="w-full max-w-md mb-5 border-2 border-border/50 bg-card/50 backdrop-blur-sm animate-in fade-in duration-300">
        ...
        {/* Players grid */}
        <Card className="w-full max-w-2xl border-2 border-border/50 bg-card/50 backdrop-blur-sm mb-6 animate-in fade-in slide-in-from-bottom-4 duration-500 delay-100">
          <CardContent className="p-4 sm:p-6">
            <h2 className="text-base sm:text-lg font-semibold text-center mb-3 sm:mb-4 text-muted-foreground">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {players.map((player, index) => (
                <div key={player.id} className="animate-in fade-in zoom-in-95 duration-300" style={{ animationDelay: `${index * 100}ms` }}>
                  <div className="relative flex flex-col items-center p-3 rounded-xl bg-muted/30 border-2 border-border/50 hover:border-primary/50 transition-all duration-300 hover:scale-105">
                    ...
                    <p className="font-semibold text-xs sm:text-sm truncate max-w-full">{player.pseudo}</p>
                    {player.id === currentPlayer?.id && <span className="text-[10px] sm:text-xs text-primary mt-0.5">(toi)</span>}
                  </div>
                </div>
              ))}
              {Array.from({ length: Math.max(0, 3 - players.length) }).map((_, i) => (
                <div key={`empty-${i}`} className="flex flex-col items-center justify-center p-3 rounded-xl border-2 border-dashed border-border/30 opacity-50">
        ...
        <div className="flex flex-col sm:flex-row gap-3 animate-in fade-in slide-in-from-bottom-6 duration-500 delay-200 w-full max-w-md">
          {isHost ? (
            ...
            <div className="flex-1 flex items-center justify-center gap-3 px-6 py-3 rounded-full bg-muted/50 border-2 border-border/50">
              ...
              <span className="text-sm sm:text-base text-muted-foreground">En attente du chef de salon...</span>
            </div>
          )}
          <Button onClick={onLeave} variant="outline" size="lg"
            className="h-12 sm:h-14 px-6 text-base font-semibold border-2 hover:bg-destructive/10 hover:border-destructive hover:text-destructive transition-all duration-300"
          >
