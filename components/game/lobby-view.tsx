...
import type { Player, MemeLibrary, MemePack } from "@/app/page"
...
export function LobbyView({
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-8">
      ...
      {/* Host controls - Meme Pack Selection */}
      {isHost && (
        <Card className="w-full max-w-md mb-6 border-2 border-primary/50 bg-gradient-to-r from-primary/10 to-accent/10 animate-in fade-in slide-in-from-top-6 duration-500">
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button 
                    variant="outline" 
                    className="min-w-[180px] justify-between border-2 border-border hover:border-primary"
                  >
                    <span className="truncate">
                      {selectedPack ? selectedPack.name : "Choisir un pack..."}
                    </span>
            ...
            {selectedPack && selectedPack.memes.length < 3 && (
              <p className="text-xs text-destructive mt-2 text-center">
                Ce pack doit contenir au moins 3 memes
              </p>
            )}
          </CardContent>
        </Card>
      ...
      <Card className="w-full max-w-2xl border-2 border-border/50 bg-card/50 backdrop-blur-sm mb-8 animate-in fade-in slide-in-from-bottom-4 duration-500 delay-100">
        <CardContent className="p-6">
          <h2 className="text-lg font-semibold text-center mb-4 text-muted-foreground">
            Joueurs en attente ({players.length}/8)
          </h2>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {players.map((player, index) => (
              ...
              >
                <div className="relative flex flex-col items-center p-4 rounded-xl bg-muted/30 border-2 border-border/50 hover:border-primary/50 transition-all duration-300 hover:scale-105">
                  {player.isHost && (
                    <Crown className="absolute -top-2 -right-2 h-5 w-5 text-secondary animate-bounce" />
                  )}
            ...
            {Array.from({ length: Math.max(0, 3 - players.length) }).map((_, i) => (
              <div
                key={`empty-${i}`}
                className="flex flex-col items-center justify-center p-4 rounded-xl border-2 border-dashed border-border/30 opacity-50"
              >
