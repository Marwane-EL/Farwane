...
export function LobbyView({
  ...
  const canStart = selectedPack && selectedPack.memes.length >= 3 && players.length >= 2 && hasEnoughNiches
    ...
    <div className="h-full w-full flex flex-col overflow-hidden">
        ...
        <div className="flex flex-col items-center px-4 py-4 sm:py-6 min-h-fit">
          ...
          {isHost && (
            <div className="w-full max-w-md space-y-3 mb-5 animate-in fade-in slide-in-from-top-6 duration-500">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <Button
                        ...
                        className="h-10 font-black text-xs sm:text-sm gap-1.5"
                      >
                        <span>🎭</span> Classique
                      </Button>
                      <Button
                    ...
                    {settings.gameMode === "niche" && (
                      <p className="text-[11px] text-muted-foreground font-medium">
                        Chaque manche tirera au sort une niche parmi le pool avec une loterie animée !
                      </p>
                    )}
                  </div>
                    ...
                    <div className="flex gap-2 items-center">
                      ...
                      <div className="flex-1 flex items-center relative">
                        <input
                          ...
                          placeholder="Perso"
                          className={`w-full h-9 px-2 text-center rounded-md border-2 font-black text-xs sm:text-sm outline-none transition-all ${
              ...
              {/* Niche pool */}
              <Card className={`border-2 shadow-[4px_4px_0px_var(--border)] transition-all ${
                settings.gameMode === "niche"
            ...
            <Card className="w-full max-w-md mb-5 border-2 border-border shadow-[3px_3px_0px_var(--border)] animate-in fade-in duration-300">
              <CardContent className="p-3 sm:p-4 space-y-2">
                ...
                <div className="flex items-center gap-4 text-sm text-muted-foreground font-medium flex-wrap">
                  ...
                  <span className="flex items-center gap-1"><RefreshCw className="h-4 w-4" /> {settings.maxRefreshes} rerolls</span>
                  <span className="flex items-center gap-1">
                    {settings.gameMode === "niche" ? "🎯 Mode Niches" : "🎭 Mode Classique"}
                  </span>
                </div>
      ...
      <div className="shrink-0 px-4 pb-3 pt-2 sm:pb-4 border-t-2 border-border bg-background">
        <div className="flex flex-col gap-2 w-full max-w-md mx-auto">
          ...
          {isHost && settings.gameMode === "niche" && !hasEnoughNiches && (
            <div className="flex items-center justify-center gap-1.5 text-xs font-bold text-secondary text-center py-0.5">
              <AlertCircle className="h-3.5 w-3.5 shrink-0" />
              <span>Ajoutez encore {minRequiredNiches - nichePool.length} niche(s) pour lancer en Mode Niches</span>
            </div>
          )}

          <div className="flex flex-col sm:flex-row gap-2 sm:gap-3 w-full animate-in fade-in slide-in-from-bottom-4 duration-500 delay-200">
            {isHost ? (
              <Button
